...
const navLinks = [
  ...
  { href: '/dashboard/create-slider', label: 'Sliders', icon: Sliders },
  { href: '/dashboard/create-common-game-fields', label: 'Common Game Fields', icon: FileSliders },
...
const handleClickOutside = (e: MouseEvent) => {
  ...
  return (
    <aside
      className={`transition-all duration-300 ease-in-out border-r border-gray-300 ${isOpen ? 'w-64' : 'w-16'
        } bg-gray-200 text-gray-800 h-screen shadow-md flex flex-col justify-between`}
    >
      {/* Header */}
        ...
        {navLinks.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link href={href} key={href}>
              <div
                className={`flex items-center px-4 py-3 mx-2 rounded-lg cursor-pointer transition-colors ${isActive
