...
interface ListingField {
  id: number;
  label: string;
  value: string; // value seems to be text (checking `.length` in code)
  icon?: string;
}
    ...
    return (
        <div className="min-h-screen bg-[#1A1D20] text-white px-4 py-10">
            <div className="max-w-6xl mx-auto">
                <h1 className="text-4xl font-extrabold mb-12 text-center">🎮 Game Listings</h1>

                {listings.length === 0 ? (
                    <p className="text-center text-gray-400">No listings found for this game.</p>
                ) : (
                                ...
                                <div className="p-6 flex flex-col space-y-4">
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                        {listing.fields?.map((field: ListingField, idx: number) => {
                                            const isLong = (field.value?.length || 0) > 20;

                                            return (
                                                <div
                                                    key={idx}
                                                    className={`flex items-start space-x-2 bg-[#2C3034] px-4 py-3 rounded-md text-sm ${isLong ? 'text-base py-4' : ''
                                                        }`}
                                                >
                                                    {field.icon ? (
                                                        <Image
                                                    ...
                                                    <span className="text-gray-300 leading-relaxed">
                                                        <span className="text-white font-medium">{field.label}:</span> {field.value}
                                                    </span>
                                                </div>
                                            );
                                        })}
