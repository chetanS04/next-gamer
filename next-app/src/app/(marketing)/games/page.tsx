  ...
  return (
    ...
    >
      ...
      <nav className="w-full px-0 md:px-4 py-3 relative z-50">
        ...
        >
          {/* Logo */}
          <Link href="/" className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-sky-600 to-blue-400 tracking-tight drop-shadow hover:scale-105 transition focus:outline-none">
            <span style={{ letterSpacing: "2px" }}>🎮 GameHub</span>
          </Link>

          {/* Desktop Links */}
          <div className="hidden md:flex items-center gap-7">
            <Link href="/contact-us" className="px-3 py-1.5 rounded-xl font-semibold text-gray-200 hover:bg-[#053345]/50 hover:border-[#369dcc]/30 shadow-sm hover:scale-105 transition duration-100">Contact</Link>
            <Link href="/about" className="px-3 py-1.5 rounded-xl font-semibold text-gray-200 hover:bg-[#212529]/50 hover:border-[#222] hover:scale-105 shadow-sm duration-100">About</Link>
            {user ? (
              <div className="relative" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  ...
                  >
                    <button onClick={handleDashboardRedirect} className="px-3 py-2 w-full text-left font-semibold text-[#81e8ff] hover:bg-[#053345]/50 rounded transition">Dashboard</button>
                    <div className="px-3 py-1 text-xs text-gray-400 italic select-none">Role: {user?.role}</div>
                    <button onClick={handleLogout} className="px-3 py-2 w-full text-left text-red-400 font-semibold hover:bg-[#053345]/40 rounded transition">Logout</button>
                  </div>
                )}
              </div>
            ) : (
              <div className="flex items-center gap-3 ml-3">
                <Link href="/login" className="px-4 py-1 rounded-full font-semibold border border-[#063c53]/40 bg-gradient-to-tr from-[#212529] to-[#053345] text-[#b8c9d8] hover:text-white hover:bg-[#0a3650] shadow hover:scale-105 transition">Login</Link>
                <Link href="/register" className="px-5 py-1.5 rounded-full font-bold bg-gradient-to-tr from-[#053345] to-[#2b5e7d] text-sky-100 hover:text-white hover:brightness-110 transition border-none outline-none shadow-md">Register</Link>
              </div>
            )}
          </div>
        </div>
      ...
      <section className="max-w-4xl mx-auto px-4 py-10 text-center">
        <h1
          ...
          style={{
            backgroundImage: "linear-gradient(90deg, #b8fcff, #2ca0e6 60%, #053345 100%)",
          }}
        >
        ...
        <p className="text-lg md:text-xl text-gray-200 mt-2 max-w-2xl mx-auto">
          <span className="font-semibold" style={{ color: "#7fd3f7" }}>Earn money</span> by connecting your games with players worldwide.
          <br />
          <span style={{ color: "#2ca0e6" }}>List your favorite games</span> easily and securely—our trusted marketplace is built <b>for gamers, by gamers!</b>
        </p>
      </section>
        ...
        {[
          {
            ...
            desc: <>Only verified sellers and buyers.<br />Secure payments and robust privacy.</>,
          ...
          },
          {
            ...
            desc: <>Our team is here for you all day, every day.<br />Quick, friendly help at any time.</>,
          ...
          },
          {
            ...
            desc: <>Get your earnings quickly after a sale.<br />No unnecessary delays—start cashing out!</>,
        ...
        <div className="max-w-6xl mx-auto px-4">
          {loading ? (
            ...
            <div className="text-center text-gray-400 mt-28 text-xl">
              ⚠️ No games available to sell yet.<br />Please check back soon!
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 pb-6">
              {games.map((game) => (
                ...
                >
                  <div className="relative w-full h-48" style={{ background: accentBg }}>
                    {game.primary_image ? (
                      <Image
                      ...
                      <span className="text-6xl text-gray-700 opacity-50">🎮</span>
                    )}
                  ...
                  <div className="flex-1 flex flex-col p-4">
                    <h2 className="font-bold text-xl text-[#72c4e9] group-hover:text-[#bff6ff] transition">{game.name}</h2>
                    <p className="text-gray-300 text-sm mb-4 mt-2 flex-1">{game.description}</p>
                    <Link
                      ...
                      style={{
                        background: "linear-gradient(90deg, #1179af 40%, #0c2129 100%)",
                        color: "#cafdff",
      ...
      >
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <span className="text-xl" style={{ color: "#b2f6ff", fontWeight: "bold" }}>GameHub</span>
            <span className="text-gray-400 text-xs">© {new Date().getFullYear()} All Rights Reserved.</span>
          </div>
          <div className="flex gap-4 items-center">
            <Link href="/about" className="text-gray-300 hover:text-[#55c2df] text-sm">About</Link>
            <Link href="/contact-us" className="text-gray-300 hover:text-[#bbfffd] text-sm">Contact</Link>
            <a href="#" className="text-gray-300 hover:text-cyan-300 text-sm">Privacy</a>
            <a href="#" className="text-gray-300 hover:text-[#a8e1e8] text-sm">Terms</a>
          </div>
        </div>
        <div className="text-center text-xs mt-3" style={{ color: "#4bbbe7" }}>
          Crafted with <span style={{ color: "#d6fcff" }}>♥</span> for Gamers | Powered by GameHub Platform
        </div>
      </footer>
    </div>
  );
