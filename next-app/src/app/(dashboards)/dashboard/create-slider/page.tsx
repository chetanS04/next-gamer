...
const schema = yup.object({
  ...
  image: yup.mixed<File | string>().notRequired()
    ...
    .test('fileType', 'Unsupported file format. (jpg, jpeg, png, webp)', (value) =>
...
export default function SliderManagement() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (response.data.status === 'success') {
        setSuccessMessage(`${selectedSlider ? 'Updated' : 'Created'} slider successfully!`);
        fetchSliders()
  ...
  return (
    <>
              ...
              {sliders.length > 0 ? (
                sliders.map((slider: Slider, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-zinc-700 dark:text-zinc-200">{slider.title}</td>
                    <td className="px-6 py-4 text-zinc-500 dark:text-zinc-400">{slider.description}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${slider.status ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                ...
                <tr>
                  <td colSpan={6} className="text-center text-zinc-400 dark:text-neutral-500 font-medium py-8 italic">
                    No sliders found.
                  </td>
                </tr>
              )}
      ...
      <Modal width='max-w-6xl' isOpen={cropModalOpen} onClose={() => setCropModalOpen(false)} title="Crop Image">
        ...
        <div className="flex justify-end gap-4 mt-4">
          <button className="px-4 py-2 bg-gray-600 text-white rounded" onClick={() => setCropModalOpen(false)}>
            Cancel
          </button>
          <button className="px-4 py-2 bg-blue-600 text-white rounded" onClick={handleCropSave}>
            Crop & Save
          </button>
        </div>
      </Modal>
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Delete Slider">
        <p>Are you sure you want to delete <strong>{selectedSlider?.title}</strong>?</p>
        <div className="flex justify-end gap-4 mt-4">
          <button onClick={() => setIsDeleteModalOpen(false)} className="bg-gray-500 px-4 py-2 text-white rounded">
            Cancel
          </button>
          <button onClick={handleDelete} className="bg-red-500 px-4 py-2 text-white rounded">
            Delete
          </button>
        </div>
      </Modal>
