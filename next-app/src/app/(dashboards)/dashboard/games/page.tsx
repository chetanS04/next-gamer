...
function AdminPlacementManagement() {
  ...
  const openModal = (game: Game | null = null) => {
    ...
    if (game) {
      ...
      setPreviewSecondary(game.secondary_image ? `${baseUrl}${game.secondary_image}` : null);
    } else {
      reset({ status: true });
    ...
    <div className="z-[999]">
      ...
      <div className="space-y-8">
        ...
        <div className="p-6 bg-zinc-50 dark:bg-neutral-900 rounded-2xl shadow flex items-center justify-between mb-6 border border-zinc-200 dark:border-neutral-700">
          <h3 className="text-2xl font-semibold text-zinc-800 dark:text-zinc-100 tracking-tight">Games</h3>
          <button
          ...
          <table className="w-full min-w-[900px] text-sm text-left">
            ...
            <tbody className="divide-y divide-zinc-200 dark:divide-neutral-700">
              {games.length ? (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      {game.secondary_image ? (
                        <Image
                          src={`http://localhost:8000/storage/${game.secondary_image}`}
                          alt={game.name}
                              width={600}
                          height={600}
                          className="h-14 w-14 object-cover rounded border border-zinc-300 dark:border-neutral-700 shadow-sm"
                    ...
                    <td className="px-6 py-4 text-zinc-700 dark:text-zinc-100">{game.name}</td>
                    <td className="px-6 py-4 text-zinc-500 dark:text-zinc-300">{game.description}</td>
                    <td className="px-6 py-4">
                      <span
                    ...
                    <td className="px-6 py-4 text-end space-x-1">
                      ...
                      >
                        ...
                        <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" className="inline">
                          <path d="M4 13V11M8 13v-6M12 13v-3" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        Edit
                      </button>
                      <button
                      ...
                      >
                        {/* Trash icon */}
                        <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" className="inline">
                          <path d="M6 12h4M4 4h8l-1 10H5L4 4zm2-2h4v2H6V2z" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        Delete
                      </button>
                      <button
                      ...
                      >
                        ...
                        <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" className="inline">
                          ...
                          <path d="M8 6v2l1 1" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        Detail
                      </button>
                    </td>
                ...
                <tr>
                  <td colSpan={7} className="text-center text-zinc-400 dark:text-neutral-500 font-medium py-8 italic">
                    <div className="flex flex-col items-center gap-2">
                      <svg width="48" height="48" fill="none" viewBox="0 0 48 48">
                        <ellipse cx="24" cy="40" rx="16" ry="3" fill="#eee" />
                        <rect x="14" y="14" width="20" height="14" rx="5" fill="#E5E7EB" />
                        <rect x="16" y="17" width="16" height="2" rx="1" fill="#D1D5DB" />
                        <rect x="16" y="21" width="12" height="2" rx="1" fill="#D1D5DB" />
                      </svg>
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {/* Description */}
          <div>
            <label htmlFor="description" className="block text-sm font-medium">Description<span className="text-red-600">*</span></label>
            <input
              id="description"
          ...
          <div>
            ...
            />
            {errors.primary_image && <p className="text-xs text-red-500">{errors.primary_image.message}</p>}
            {previewPrimary && (
              <Image src={previewPrimary}     width={600}
                          height={600} alt="Primary Preview" className="mt-2 h-24 w-24 rounded object-cover border" />
            )}
          </div>
          ...
          <div>
            ...
            {previewSecondary && (
              <Image src={previewSecondary}     width={600}
                          height={600} alt="Secondary Preview" className="mt-2 h-24 w-24 rounded object-cover border" />
            )}
          </div>
  ...
  <label htmlFor="status" className="flex items-center gap-3 cursor-pointer">
    ...
    >
      ...
      ></div>
          ...
          <button
            type="submit"
            className={`w-full rounded-lg p-3 text-white ${isFormSubmit ? 'bg-gray-400 cursor-not-allowed' : 'bg-blue-500 hover:bg-blue-600'}`}
      ...
      >
        ...
        <div className="mt-4 flex justify-end space-x-4">
          <button onClick={() => setIsDeleteModalOpen(false)} className="rounded bg-gray-500 px-4 py-2 text-white">Cancel</button>
          <button onClick={handleDelete} className="rounded bg-red-500 px-4 py-2 text-white">Delete</button>
        </div>
      </Modal>
    </div>
  );
